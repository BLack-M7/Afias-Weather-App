export default async function handler(req, res) {
  ...
  try {
    ...
    const weatherEndpoint = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${apiKey}&units=metric`;
    ...
    if (lat == null || lon == null) return res.status(500).json({ error: 'No coordinates found' });
